...
def --env init [
  ...
  use ./lib.nu is-using-home-manager
  if (is-using-home-manager) and not $force {
    error make -u {msg: $"Error: home-manager is currently active. Use --force to override"}
  }
...
module _main {
  ...
  export def --wrapped build [
    ...
    let profile = (init --profile=$profile --dir=$dir --force=$force)
  ...
  export def --wrapped develop [
    ...
    init --profile=$profile --dir=$dir --force=$force
    nix develop ...$args
  }
  ...
  # flakey-profile
  export def --wrapped main [cmd ...args] {
